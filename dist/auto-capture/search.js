...
const trackedSearchInputs = new WeakSet();
const getSearchQuery = (input) => {
    return input.value.trim();
};
const captureSearch = (query) => {
    if (query.length < 2)
        return; // Filter noise
...
const handleFormSubmit = (event) => {
    ...
    const searchInput = form.querySelector('input[type="search"], input[data-tf-search]');
    if (searchInput) {
        if (searchDebounceTimer)
            clearTimeout(searchDebounceTimer);
...
const bindSearchInput = (input) => {
    if (trackedSearchInputs.has(input))
        return;
    // Ignore if explicitly told to
    if (input.hasAttribute('data-tf-ignore'))
        return;
    trackedSearchInputs.add(input);
    input.addEventListener('input', handleInput);
    // If it's part of a form, bind to form submit as well
    if (input.form && !trackedSearchInputs.has(input.form)) {
        trackedSearchInputs.add(input.form);
